...
public interface TaskChangeMapper extends BaseMapper<TaskChangePo> {
    ...
    @Select("SELECT pt.pid,p.`name` AS `project`,pt.vid,pv.version,pt.id AS tid,pt.task,tc.beforeStartTime,tc.beforeEndTime,tc.afterStartTime,tc.afterEndTime,tc.reason,tc.uid,u.nickName,tc.auditorName,tc.createTime FROM `task_change` AS tc LEFT JOIN project_task AS pt ON tc.taskId = pt.id LEFT JOIN project AS p ON p.id = pt.pid LEFT JOIN project_version AS pv ON pv.id = pt.vid LEFT JOIN `user` AS u ON u.id = tc.uid ${ew.customSqlSegment} ")

...
//    List<ProjectBo> selectByName(ProjectPo projectPo, @Param(Constants.WRAPPER) Wrapper ew);
     ...
     */
    @Select("SELECT p.id, p.`name`, pv.version, p.uid, u.nickName, p.`status`, p.remark, p.updateTime, p.createTime FROM `project` AS `p` LEFT JOIN `user` AS `u` ON p.uid = u.id LEFT JOIN project_version AS pv ON p.id = pv.pid ${ew.customSqlSegment}")
     ...
     * 活动中的项目任务情况统计
     */
    @Select("SELECT t1.id, t1.`name`, t1.version, SUM(if(ISNULL(t1.`status`),0,t1.numb)) as taskTotal, SUM(if(t1.`status`=1,t1.numb,0)) as finishTaskNum FROM( SELECT p.id, p.`name`, p.version, t.`status`, count(*) AS numb FROM `project` AS p LEFT JOIN project_task AS t ON p.id = t.pid WHERE p.`status`=4 GROUP BY p.id, p.`name`, p.version, t.`status`) AS t1 GROUP BY t1.id, t1.`name`, t1.version ORDER BY t1.`name`, t1.version")

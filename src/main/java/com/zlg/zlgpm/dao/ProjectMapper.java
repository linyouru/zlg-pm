...
//    List<ProjectBo> selectByName(ProjectPo projectPo, @Param(Constants.WRAPPER) Wrapper ew);
     ...
     */
    @Select("SELECT p.id, p.`name`, pv.version, p.uid, u.nickName, p.`status`, p.remark, p.link, p.updateTime, p.createTime FROM `project` AS `p` LEFT JOIN `user` AS `u` ON p.uid = u.id LEFT JOIN project_version AS pv ON p.id = pv.pid ${ew.customSqlSegment}")
